...
def save_to_csv(log_file, data):
    ...
    with open(log_file, 'a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(['Data', 'Hora', 'Usuario', 'Status', 'Lista', 'Mensagem', 'Arquivos'])
...
def main(page: ft.Page):
    ...
    arquivos_selecionados = []  # Agora é uma lista para múltiplos arquivos
    lista_editando = None
    ...
    def atualizar_lista_arquivos():
        ...
        if not arquivos:
            ...
            for arquivo in arquivos:
                ...
                is_selected = arquivo in arquivos_selecionados
                
                # Criar botão para selecionar/deselecionar arquivo
                btn = ft.ElevatedButton(
                    content=ft.Row([
                        ft.Text("✅ " if is_selected else icon, size=16),
                    ...
                    on_click=lambda e, a=arquivo: alternar_selecao_arquivo(a),
                )
                
                arquivos_container.controls.append(btn)
        
        atualizar_info_arquivos()
        page.update()
    
    def alternar_selecao_arquivo(arquivo_path):
        """Alterna a seleção de um arquivo"""
        if arquivo_path in arquivos_selecionados:
            arquivos_selecionados.remove(arquivo_path)
            log(f"Arquivo removido: {arquivo_path.name}", "info")
        else:
    ...
    def atualizar_info_arquivos():
        """Atualiza a informação sobre arquivos selecionados"""
        if not arquivos_selecionados:
            arquivo_info.value = "📎 Nenhum arquivo selecionado"
        elif len(arquivos_selecionados) == 1:
            arquivo_info.value = f"📎 1 arquivo selecionado: {arquivos_selecionados[0].name}"
        else:
            arquivo_info.value = f"📎 {len(arquivos_selecionados)} arquivos selecionados"
    
    def selecionar_todos_arquivos(e):
        """Seleciona todos os arquivos disponíveis"""
        arquivos = listar_arquivos_midia()
        if arquivos:
            arquivos_selecionados.clear()
            arquivos_selecionados.extend(arquivos)
            log(f"✅ Todos os arquivos selecionados ({len(arquivos)} total)", "success")
            atualizar_lista_arquivos()
    
    def deselecionar_todos_arquivos(e):
        """Desseleciona todos os arquivos"""
        if arquivos_selecionados:
            log(f"🗑️ Todos os arquivos desselecionados", "system")
            arquivos_selecionados.clear()
            atualizar_lista_arquivos()
    ...
    def update_dashboard():
        ...
        cards_data = [
            ...
            ("✅ Selecionados", f"{len(arquivos_selecionados)}", COLORS["warning"]),
        ]
    ...
    def enviar_mensagens(e):
        """Função principal de envio de mensagens com múltiplos arquivos"""
        # Validar seleção
        selecionadas = [
        ...
        def worker():
            try:
                ...
                if arquivos_selecionados:
                    log(f"📎 Enviando {len(arquivos_selecionados)} arquivo(s) anexado(s)", "info")
                log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "system")
                ...
                if client:
                    ...
                    try:
                        ...
                        for user in usuarios_slack:
                            ...
                            try:
                                ...
                                # Enviar mensagem com múltiplos arquivos se houver
                                if arquivos_selecionados:
                                    # Para múltiplos arquivos, precisamos enviar um a um
                                    for i, arquivo in enumerate(arquivos_selecionados):
                                        if arquivo.exists():
                                            with open(arquivo, 'rb') as file:
                                                if i == 0:
                                                    # Primeiro arquivo inclui a mensagem
                                                    ...
                                                    # Arquivos subsequentes apenas o arquivo
                                                    response = client.files_upload_v2(
                                                        channel=dm["channel"]["id"],
                                                        file=file,
                                                        filename=arquivo.name
                                ...
                                # Log e CSV
                                nomes_arquivos = ", ".join([a.name for a in arquivos_selecionados]) if arquivos_selecionados else ""
                                log_data = [
                    ...
                    for i, usuario in enumerate(usuarios_para_enviar, 1):
                        ...
                        log_msg = f"✅ [{i}/{len(usuarios_para_enviar)}] SIMULAÇÃO para {usuario.title()}"
                        if arquivos_selecionados:
                            log_msg += f" com {len(arquivos_selecionados)} arquivo(s)"
                        log(log_msg, "success")
                        total_enviados += 1
                ...
                if arquivos_selecionados:
                    log(f"   • Arquivos anexados ({len(arquivos_selecionados)}):", "info")
                    for arquivo in arquivos_selecionados:
                        log(f"     - {arquivo.name}", "info")
        ...
        content=ft.Row(
            [
                ...
                ft.Column([
                    ...
                    ft.Text("Desenvolvido por Tiago de Abreu | @devtiagoabreu", size=12, color=COLORS["text"], opacity=0.6),
                ]),
    ...
    # Área de log
    log_area = ft.ListView(spacing=5, padding=10, auto_scroll=True, height=250)
    ...
    editor_dialog = ft.AlertDialog(
        ...
        actions=[
            ft.TextButton("Cancelar", on_click=lambda e: (setattr(editor_dialog, 'open', False), page.update())),
                        ...
                        ft.Container(
                            ...
                            content=ft.Column(
                                [
                                    ...
                                    # Arquivos - AGORA COM SELEÇÃO MÚLTIPLA
                                    ft.Text("📎 ARQUIVOS E MÍDIA", size=16, weight=ft.FontWeight.BOLD),
                                    arquivo_info,
                                    ft.Row([
                                        ft.OutlinedButton(
                                            "✅ Selecionar Todos",
                                            on_click=selecionar_todos_arquivos,
                                        ),
                                        ft.OutlinedButton(
                                            "🗑️ Limpar Seleção",
                                            on_click=deselecionar_todos_arquivos,
                                        ),
                                        ft.OutlinedButton(
                                            "🔄 Atualizar",
                                            on_click=lambda e: atualizar_lista_arquivos(),
                                        ),
                                    ]),
                                    ft.Container(
                                    ...
                                    ft.Column([
                                        ...
                                        ft.Row([
                                            limpar_log_btn,
                                            ft.Container(expand=True),
                                            ft.Text(f"v1.4.0 | @devtiagoabreu", size=10, color=COLORS["text"], opacity=0.5),
                                        ]),
                                    ], spacing=0),
                                ],
                                spacing=15,
                                    ...
                                    ft.Column(
                                        [
                                            ft.Row([
                                                ...
                                                ft.Container(expand=True),
                                                ft.Text("Caracteres: 0", size=12, color=COLORS["text"], opacity=0.7),
                                            ]),
                                            ft.Container(
    ...
    def atualizar_contador_caracteres(e):
        # Encontrar o contador no layout
        for child in page.controls[0].controls:
            if isinstance(child, ft.Column):
                for row in child.controls:
                    if isinstance(row, ft.Row) and len(row.controls) > 1:
                        for col in row.controls:
                            if isinstance(col, ft.Container):
                                # Procurar o texto do contador
                                def encontrar_contador(control):
                                    if isinstance(control, ft.Text) and "Caracteres:" in control.value:
                                        control.value = f"Caracteres: {len(mensagem_input.value or '')}"
                                        return True
                                    if hasattr(control, 'controls'):
                                        for c in control.controls:
                                            if encontrar_contador(c):
                                                return True
                                    return False
                                
                                encontrar_contador(col)
        
        page.update()
    
    # Adicionar listener ao campo de mensagem
    mensagem_input.on_change = atualizar_contador_caracteres
...
if __name__ == "__main__":
    print("🚀 Iniciando Slack DM Manager Pro v1.4.0...")
    ...
    print("\nDesenvolvido por Tiago de Abreu | @devtiagoabreu")
    print("Sistema completo de envio de mensagens para Slack")
    print("✨ NOVA FUNCIONALIDADE: Seleção múltipla de arquivos!")
