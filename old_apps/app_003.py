...
def main(page: ft.Page):
    ...
    def handle_upload_result(result):
        """Processa o resultado do upload de arquivos - CORRIGIDO: sem tipagem"""
        if result.files:
    ...
    def handle_import_result(result):
        """Processa o resultado da importação de listas - CORRIGIDO: sem tipagem"""
        if result.files:
            for file in result.files:
    ...
    # Controles de envio - MENSAGEM AGORA OCUPA MAIS ESPAÇO
    mensagem_input = ft.TextField(
        label="Digite sua mensagem",
        multiline=True,
        min_lines=15,  # AUMENTADO
    ...
    # Área de log - MENOR PARA DAR ESPAÇO À MENSAGEM
    log_area = ft.ListView(spacing=5, padding=10, auto_scroll=True, height=250)  # ALTURA FIXA
    ...
    dialog_criar_lista = ft.AlertDialog(
        ...
        content=dialog_nome,
        actions=[
            ft.TextButton("Cancelar", on_click=lambda e: (setattr(dialog_criar_lista, 'open', False), page.update())),
            ft.ElevatedButton("Criar", on_click=confirmar_criar_lista, bgcolor=COLORS["success"]),
        ],
    )
    
    # =========================
    # LAYOUT PRINCIPAL COM ESPAÇO OTIMIZADO
    # =========================
    page.add(
        ft.Column(
            [
                ...
                # Corpo principal com layout mais flexível
                ft.Row(
                    [
                                    ...
                                    ft.Row([
                                        ...
                                        ft.OutlinedButton(
                                            ...
                                            on_click=lambda e: page.pick_files(
                                                ...
                                                allow_multiple=True,
                                                on_result=handle_import_result
                                            )
                                        ),
                                    ]),
                                    ft.Container(
                        ...
                        # Coluna direita (mensagem e logs) - OCUPA TODO O ESPAÇO DISPONÍVEL
                        ft.Container(
                            content=ft.Column(
                                [
                                    # ÁREA DA MENSAGEM (EXPANDE PARA OCUPAR ESPAÇO)
                                    ft.Column(
                                        [
                                            ft.Row([
